"""
03_outputs.py — Durango Carbon Pricing Overlap Analysis
=========================================================
Produces publication-quality figures and summary tables.

Outputs:
    outputs/figures/durango_venn_segments_2025_2026.png
    outputs/figures/durango_coverage_summary_2025_2026.png
    outputs/tables/durango_overlap_summary.csv
    outputs/tables/durango_overlap_full_table.csv
"""
...
WB_GREY  = "#6D6E71"
WB_LGREY = "#E8E8E8"
TIER_COL = "#C8102E"   # Tier 3 flag colour

# Venn segment colours
SEG_COLS = {
    "S∩F∩E":    "#6B3FA0",   # all three — purple
    "S∩E only": "#00538C",   # state + ETS (NG-dominant) — WB blue
    "S∩F only": "#00A9A5",   # state + federal (small manuf) — teal
    "S only":   "#7EB8D4",   # state only (HFCs, NG small, fugitives) — light blue
    "F only":   "#E7A720",   # federal only (transport) — gold
    "Uncovered":"#D9D9D9",   # grey
}

# ── Load data ─────────────────────────────────────────────────────────────────
overlap  = pd.read_csv(os.path.join(PROC_DIR, "durango_overlap_estimates.csv"))
scope_df = pd.read_csv(os.path.join(PROC_DIR, "durango_tax_scope_2022.csv"))

def get_yr_scen(yr, scen):
    return overlap[(overlap["year"]==yr) & (overlap["scenario"]==scen)].iloc[0]

# ── Figure 1: Venn segment bar chart (2025 + 2026, central + range) ──────────
fig, axes = plt.subplots(1, 2, figsize=(12, 6), sharey=True)
fig.suptitle(
    "Durango State Carbon Tax × Federal Carbon Tax × Mexico Pilot ETS\n"
...
# Tier note
fig.text(0.5, -0.04,
    "Tier 3 estimation: ETS threshold via Pareto analysis per manufacturing subsector. "
    "Federal tax coverage excludes NG (99.6% of electricity CO₂).\n"
...
fig.tight_layout()
out1 = os.path.join(FIG_DIR, "durango_venn_segments_2025_2026.png")
fig.savefig(out1, dpi=150, bbox_inches="tight")
plt.close()
print(f"Saved: {out1}")

# ── Figure 2: Coverage summary — stacked horizontal bars ─────────────────────
fig2, ax2 = plt.subplots(figsize=(10, 5.5))
fig2.suptitle(
    "Durango Carbon Pricing Coverage Summary\nThree Instruments, 2022–2026 (GgCO₂e)",
...
for i, (inst, key, col) in enumerate(zip(INSTRUMENTS, INST_KEYS, INST_COLS)):
    ...
    errs_lo  = [c-l for c,l in zip(centrals, lows)]
    errs_hi  = [h-c for c,h in zip(centrals, highs)]
...
# Key analytical note box
note = (
    "Key finding: Durango state tax covers ~34.6% of state emissions (2022).\n"
    "Unlike Colima/federal tax, NG IS in scope — so [1A1] electricity is fully\n"
    "in S. Federal tax covers only ~24.9% because NG dominates electricity (99.6%).\n"
    "Transport (~21.9% of state) is in F only, not S. HFCs, fugitives in S only."
)
...
fig2.text(0.5, -0.03,
    "Source: IEEGYCEI Durango 2022 (Centro Mario Molina / SRNMA). "
    "Estimation tier: Tier 3 (ETS threshold + fuel fractions). "
    "GWPs: AR5 (CH₄=28, N₂O=265).",
    ha="center", fontsize=7, style="italic", color=WB_GREY)

fig2.tight_layout()
out2 = os.path.join(FIG_DIR, "durango_coverage_summary_2025_2026.png")
...
summary_tbl.to_csv(os.path.join(TBL_DIR, "durango_overlap_summary.csv"), index=False)
print(f"Saved: outputs/tables/durango_overlap_summary.csv")

# ── Table 2: Full table for publication ──────────────────────────────────────
rows_pub = []
SEGS_PUB = [
    ("S∩F∩E — all three instruments", "S_F_E_GgCO2e", "S_F_E_pct",
     "Elec (diesel/FO fraction) + manuf (taxable fuels at large plants)"),
    ("S∩E only — state tax + ETS", "S_E_only_GgCO2e", "S_E_only_pct",
     "Electricity (NG fraction, 99.6%) + manufacturing NG at large plants"),
    ("S∩F only — state tax + federal", "S_F_only_GgCO2e", "S_F_only_pct",
     "Manufacturing (taxable fuels, below ETS threshold); commercial/residential taxable"),
    ("S only — state tax unique", "S_only_GgCO2e", "S_only_pct",
     "NG emissions at small facilities; HFCs [2F]; fugitives [1B2] non-ETS; metal processes non-ETS"),
    ("F only — federal tax unique", "F_only_GgCO2e", "F_only_pct",
     "Transport (road, rail, aviation) — 1A3; agricultural combustion"),
    ("Uncovered", "uncovered_GgCO2e", "uncovered_pct",
     "ASOUT (livestock, land non-CO2); Waste sector; NG commercial/residential"),
]
...
full_tbl.to_csv(os.path.join(TBL_DIR, "durango_overlap_full_table.csv"), index=False)
print(f"Saved: outputs/tables/durango_overlap_full_table.csv")

# ── Console summary ───────────────────────────────────────────────────────────
print("\n" + "="*65)
print("DURANGO — FINAL PUBLICATION SUMMARY (2025 CENTRAL)")
print("="*65)
...
print(f"  S only:            {yr25c['S_only_GgCO2e']:,.0f}  ({yr25c['S_only_pct']:.1f}%)  ← HFCs, fugitives, small NG")
print(f"  F only:            {yr25c['F_only_GgCO2e']:,.0f}  ({yr25c['F_only_pct']:.1f}%)  ← Transport")
...
print("\n  Structural finding:")
print("  • State tax adds substantial unique coverage vs Colima — no NG exemption")
print("    means entire electricity sector (3,300 GgCO2e) is in scope for S")
print("  • Federal tax covers very little of electricity (NG=99.6% → only 0.4% taxable)")
print("  • Transport (~2,900 GgCO2e) is F-only — state tax transport exemption is large")
print("  • Dominant overlap is S∩E: NG-fired power plants in both state tax and ETS")
